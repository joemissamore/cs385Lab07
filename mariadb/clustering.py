...
def main():
    ...
    try:
        ...
        ipaddresses = zk.get(CLUSTER_NODES)[0].decode().split()
        zk.set(CLUSTER_NODES, '{},{}'.format(ipaddresses, get_ip_address()).encode())

        # If the initial node exists
        if (zk.exists):
            print("I am not the first")
            start_node(ipaddresses)
       ...
       # Wait until the node has joined the galera cluster 
        wait_until_primary()
